...
class Scheduler(object):
	...
	def RegisterAutonomousTask(self, taskName, task, ttype=SEQUENTIAL_TASK, *params):
		...
		self.auto_tasks.append([taskName, ttype, task, params, None, None])


	def RegisterAutonomousTimedTask(self, taskName, task, timeToRun, ttype=SEQUENTIAL_TASK, *params):
		"""
		Register an autonomous task to be executed in order they were added.
		Autonomous tasks can be either a SEQUENTIAL_TASK or a PARALLEL_TASK.
		...
		# Store the task as a list with it's name and ttype.
		self.auto_tasks.append([taskName, ttype, task, params, timeToRun, None])
	...
	def RegisterAutonomousTimedTaskIfNotAlready(self, taskName, task, timeToRun, ttype=SEQUENTIAL_TASK, *params):
		"""
		Register an autonomous task to be executed in order they were added.
		Autonomous tasks can be either a SEQUENTIAL_TASK or a PARALLEL_TASK.
	...
	def Autonomous(self, debug=False):
		...
		See RemoveAutonomousTask for an explanation of the diffrent kind of tasks.
		"""
		# If we have no more tasks return True
		...
		for i, info in enumerate(self.auto_tasks):
			...
			if timeToRun != None and startTime == None:
				info[5] = time.time()
				startTime = info[5]
			...
			finished = False
			if timeToRun != None:
				finished = ((time.time() - startTime) >= timeToRun)
			if result or finished:
				# Add it to the delete list
				to_delete.append(i)
				if debug:
					print(name, "has resolved.")
