...
if __name__ == '__main__':
	...
	# Register tasks
	rc.RegisterOperatorControlTask("test", test)
	rc.RegisterOperatorControlTask("test2", test2, "Jacob")

	# "Teleop loop"
	for i in range(100):
		rc.OperatorControl(debug=True)
		rc.RemoveOperatorControlTask("test", debug=True)
	...
	class Auto(object):
		...
		def parallel1(self, name):
			self.task += 1
			print(name)
			return False

	auto = Auto()
	rc.RegisterAutonomousTask(
							  #comment
							  "sequential1", 
							  #Comment
							auto.sequential1)
	rc.RegisterAutonomousTask("parallel1", auto.parallel1, Scheduler.PARALLEL_TASK, "POOP")
	...
	for i in range(10):
		rc.Autonomous(debug=True)

	rc.ClearAutonomousTasks()

	rc.ListAutonomousTasks()
